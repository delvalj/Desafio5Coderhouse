...
let fileExists = fs.existsSync("NuevosProductosEJS.txt");
console.log("NuevosProductosEJS.txt exists:", fileExists);

// If the file does not exist
// create it
if (!fileExists) {
  console.log("Creating the file");
  fs.writeFileSync("NuevosProductosEJS.txt", "[]");
  console.log("Archivo NuevosProductosEJS.txt Creado!");
}
...
const ejecutarProductos = async () => {
  const productos = new Contenedor("NuevosProductosEJS.txt");
};
...
const productoSubido = storage.fields([
  ...
  router.post("/productos", productoSubido, async (req, res, next) => {
    const subirProduct = async () => {
      let produc = new Contenedor("NuevosProductosEJS.txt");
      if (
        req.body.title === "" ||
        req.body.price === "" ||
        req.body.thumbnail === ""
